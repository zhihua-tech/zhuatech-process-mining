...
| POST | `/domain/decision` | 执行企业流程挖掘与优化平台专属领域规则 |
...
| `totalCases` | int | 流程实例数 |
| `conformingCases` | int | 合规实例数 |
| `medianCycleHours` | double | 中位周期(小时) |
| `targetCycleHours` | double | 目标周期(小时) |
| `reworkRate` | double | 返工率(%) |
| `slaBreachRate` | double | SLA违约率(%) |
| `eventCoverage` | double | 事件覆盖率(%) |
| `ownerAssigned` | boolean | 已指定流程负责人 |
| `piiMasked` | boolean | 个人信息已脱敏 |
