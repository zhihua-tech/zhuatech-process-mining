...
package cn.zhuatech.processmining.config;
import cn.zhuatech.processmining.domain.DomainCatalog;
import cn.zhuatech.processmining.model.*;
import cn.zhuatech.processmining.repository.*;
...
@Configuration public class DataInitializer {
 @Bean CommandLineRunner seed(BusinessRecordRepository records,SystemSettingRepository settings,DomainCatalog catalog) {
  return args -> { if(records.count()>0)return;
            ...
            settings.save(new SystemSetting("recordNoRule", "PMIN-{YYYY}-{SEQ}"));
   ...
   int sequence=1; for(var module:catalog.modules()) {
    String no="PMIN-DEMO-"+String.format("%03d",sequence);
